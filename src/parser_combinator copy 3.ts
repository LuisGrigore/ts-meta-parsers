...
export const map =
  <State, ValueA, ValueB>(fn: (value: ValueA) => ValueB) =>
  (parser: Parser<State, ValueA>): Parser<State, ValueB> =>
...
type SameState<T extends readonly Parser<any, any>[]> =
...
type StateUnion<P extends readonly Parser<any, any>[]> =
  UnionToIntersection<ParserState<P[number]>>;

export const sequenceOf =
  <const T extends /*readonly*/ Parser<any, any>[]>(
    ...parsers: T
  ): Parser<SameState<T>, ValuesOf<T>> =>
  (initialState) => {
...
export const between =
  ...
  <A>(contentParser: Parser<State, A>): Parser<State, A> =>
    map<State, [L, A, R], A>((results) => results[1])(
