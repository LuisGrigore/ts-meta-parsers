...
export const chain =
  ...
  (state: State) =>
    pipe(
      parser(state),
      foldResult(
        ({ state: nextState, value }) => fn(value)(nextState),
        (err) => fail(err),
      ),
    );
...
//     pipe(
//       parser(state),
//       E.map(({ state: s, value }) => ({ state: fn(s), value })),
//     );
...
export const optional =
  <S, A>(parser: Parser<S, A>): Parser<S, A | null> =>
  (state) => {
    const result = parser(state);
...
export const sepBy =
  ...
  (parser: Parser<S, A>): Parser<S, readonly A[]> =>
  (state) => {
